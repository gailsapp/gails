...
function IsWindows() {
  return window._gails.environment.OS === "windows";
}
function IsDebug() {
  return Boolean(window._gails.environment.Debug);
}
...
window._gails = window._gails || {};
window._gails.callResultHandler = resultHandler;
window._gails.callErrorHandler = errorHandler;
...
function SetBadge(label) {
  return ByID(1717705661, label);
}
function SetCustomBadge(label, options) {
  return ByID(2730169760, label, options);
}
class RGBA {
