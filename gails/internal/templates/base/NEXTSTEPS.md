...
> runs `gails init`. Template variables (e.g. `{{.ProjectName}}`) are replaced with
...
automatically** by `gails init` after your template is applied. You do not need to
...
When a user installs a remote template, Gails displays a warning explaining that the
template is third-party code and that the Gails project takes no responsibility for
...
- Keeping your template up to date with new Gails versions
