...
type BaseTemplate struct {
	...
	GailsVersion uint8  `json:"gailsVersion" yaml:"gailsVersion" description:"The Wails major version this template targets"`
...
type Template struct {
	...
	// New templates use template.yaml with GailsVersion instead.
...
func parseTemplate(templateFS fs.FS, templateName string) (Template, error) {
	...
	if yamlErr == nil {
		...
		if result.GailsVersion == 0 {
			return result, fmt.Errorf("template.yaml must specify 'gailsVersion' (e.g. gailsVersion: 3)")
		}
		if result.GailsVersion != 3 {
			return result, fmt.Errorf("template targets Wails v%d and is not compatible with this version of Wails", result.GailsVersion)
		}
		return result, nil
	}
...
func GenerateTemplate(options *BaseTemplate) error {
	...
	// gailsVersion is mandatory in the YAML format and set to 3.
	options.GailsVersion = 3
