...
func TestParseTemplate_YAML_Valid(t *testing.T) {
	...
	if tmpl.GailsVersion != 3 {
		t.Errorf("GailsVersion = %d, want 3", tmpl.GailsVersion)
	}
}
...
func TestParseTemplate_YAML_MissingWailsVersion(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected error for missing gailsVersion, got nil")
	}
}
...
func TestParseTemplate_YAML_WrongWailsVersion(t *testing.T) {
	...
	_, err := parseTemplate(fsys, "")
	if err == nil {
		t.Fatal("expected error for gailsVersion 2, got nil")
	}
}
...
func TestGenerateTemplate_TemplateYAMLContainsWailsVersion3(t *testing.T) {
	...
	content := string(data)
	if !strings.Contains(content, "gailsVersion: 3") {
		t.Errorf("template.yaml does not contain 'gailsVersion: 3':\n%s", content)
	}
}
