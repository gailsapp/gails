...
func IOSXcodeGen(options *IOSXcodeGenOptions) error {
	...
	// Generate iOS AppIcon PNGs from build/appicon.png if present; otherwise
	// prefer assets/favicon-1024.png at the project root, and finally fall back
	// to the embedded default. The assets/favicon-1024.png path is the
	// conventional location for the default app icon.
	...
	if _, err := os.Stat(inputIcon); err == nil {
		...
		for _, candidate := range []string{
			filepath.Join("assets", "favicon-1024.png"),
			"favicon-1024.png",
		} {
			if data, rerr := os.ReadFile(candidate); rerr == nil {
				iconReader = bytes.NewReader(data)
				break
			}
		}
		if iconReader == nil {
		...
		if iconReader != nil {
			if err := generateIOSAppIconsFromReader(iconReader, assetsDir); err != nil {
