...
func generateExampleIcon() error {
	// Prefer the project-level assets/favicon-1024.png so users can override
	// the default by dropping a file at the conventional location. Fall back to
	// the embedded default (build_assets/appicon.png) when none is found.
	const defaultOutput = "appicon.png"
	for _, candidate := range []string{
		filepath.Join("assets", "favicon-1024.png"),
		"favicon-1024.png",
	} {
		if data, err := os.ReadFile(candidate); err == nil {
			return os.WriteFile(defaultOutput, data, 0644)
		}
	}
	...
	return os.WriteFile(defaultOutput, appIcon, 0644)
}
