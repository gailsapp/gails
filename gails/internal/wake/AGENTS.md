...
**Limitation**: a file embedded directly by the main package that is neither a `.go` file nor produced by a dependency's `generates` won't be tracked. The Gails template routes embedded assets through `frontend/dist` (a dep generate), so the common case is covered.
...
All packages have tests. Run from `gails-v3/v3/` directory.
