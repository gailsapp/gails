...
const _invoke = (function () {
    try {
        ...
        // Android WebView - uses addJavascriptInterface which exposes window.gails.invoke
        else if ((window as any).gails?.invoke) {
            return (msg: any) => (window as any).gails.invoke(typeof msg === 'string' ? msg : JSON.stringify(msg));
        }
    } catch(e) {}
...
export function IsWindows(): boolean {
    return (window as any)._gails?.environment?.OS === "windows";
}
...
export function IsLinux(): boolean {
    return (window as any)._gails?.environment?.OS === "linux";
}
...
export function IsMac(): boolean {
    return (window as any)._gails?.environment?.OS === "darwin";
}
...
export function IsAMD64(): boolean {
    return (window as any)._gails?.environment?.Arch === "amd64";
}
...
export function IsARM(): boolean {
    return (window as any)._gails?.environment?.Arch === "arm";
}
...
export function IsARM64(): boolean {
    return (window as any)._gails?.environment?.Arch === "arm64";
}
...
export function IsDebug(): boolean {
    return Boolean((window as any)._gails?.environment?.Debug);
}
