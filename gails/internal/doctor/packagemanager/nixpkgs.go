...
// Packages returns the libraries that we need for Gails to compile
...
func (n *Nixpkgs) Packages() Packagemap {
	...
	return Packagemap{
		// GTK4 + WebKitGTK 6.0 (default for Gails)
		"gtk4": []*Package{
		...
		// GTK3 + WebKitGTK 4.1 (legacy - opt-in via -tags gtk3, scheduled for removal)
		"gtk3 (legacy)": []*Package{
			{Name: channel + ".gtk3", SystemPackage: true, Library: true, Optional: true},
		},
		"webkit2gtk (legacy)": []*Package{
			{Name: channel + ".webkitgtk_4_1", SystemPackage: true, Library: true, Optional: true},
		},
		"gcc": []*Package{
